...
export const BAL = {
  ...
  lvK: 2.2,                // 레벨 곡선 기울기(작을수록 레벨업 느림)
  cubePerMilestone: 1,     // 사업 마일스톤 1개 달성 시 큐브(가챠 재화 주 공급원)
...
export const GENERATORS = [
  { id: 'g1',  name: '붕어빵 가판대',   baseCost: 15,     baseRev: 0.5,    mult: 1.100 },
  { id: 'g2',  name: '떡볶이 포차',     baseCost: 1.0e2,  baseRev: 3,      mult: 1.100 },
  { id: 'g3',  name: '김밥천국',        baseCost: 1.1e3,  baseRev: 30,     mult: 1.095 },
  { id: 'g4',  name: '치킨집',          baseCost: 1.2e4,  baseRev: 300,    mult: 1.090 },
  { id: 'g5',  name: '디저트 카페',     baseCost: 1.3e5,  baseRev: 3.0e3,  mult: 1.085 },
  { id: 'g6',  name: '수제버거 가게',   baseCost: 1.4e6,  baseRev: 3.0e4,  mult: 1.080 },
  { id: 'g7',  name: '패밀리 레스토랑', baseCost: 1.5e7,  baseRev: 3.0e5,  mult: 1.080 },
  { id: 'g8',  name: '호텔 뷔페',       baseCost: 1.6e8,  baseRev: 3.0e6,  mult: 1.075 },
  { id: 'g9',  name: '미슐랭 레스토랑', baseCost: 1.7e9,  baseRev: 3.0e7,  mult: 1.075 },
  { id: 'g10', name: '프랜차이즈 본사', baseCost: 1.8e10, baseRev: 3.0e8,  mult: 1.070 },
  { id: 'g11', name: '글로벌 식품기업', baseCost: 2.0e11, baseRev: 3.0e9,  mult: 1.070 },
  { id: 'g12', name: '우주 미식제국',   baseCost: 2.5e12, baseRev: 3.0e10, mult: 1.070 },
]
...
export function levelForLifetime(lifetime, xpMult = 1) {
  ...
  return Math.floor(BAL.lvK * Math.log10(v + 1))
}
export function lifetimeForLevel(level, xpMult = 1) {
  const raw = Math.pow(10, level / BAL.lvK) - 1
  return raw / (1 + xpMult)
}
...
export function buildGachaPool() {
  ...
  BASES.forEach((base, bi) => {
    for (const prefix of PREFIXES) {
      combos.push({ i: idx, base, baseIdx: bi, prefix, h: rng(idx * 2654435761 + 7) }); idx++
    }
  })
  ...
  for (const c of combos) {
    ...
    const item = {
      id: 'it' + c.i, name: `${c.prefix} ${c.base}`,
      art: 'f' + String(c.baseIdx + 1).padStart(2, '0'),
...
// ---- 테마 (눈 안 피로하게 다양) -------------------------------------------
// 각 테마 = 고유 배경 그림(bg) + 어울리는 팔레트. 전반적으로 밝고 차분하게.
export const THEMES = [
  { id: 'caramel', name: '카라멜 분식', bg: 'caramel', light: true, vars: {
    '--bg':'#fdf3e6','--bg2':'#f7ead7','--panel':'rgba(255,252,246,.90)','--panel2':'rgba(252,243,228,.95)',
    '--line':'#ecdcc4','--text':'#3c2c1d','--muted':'#9a8167','--accent':'#ef8f3c','--accent2':'#df7128','--good':'#2f9e63','--glow':'rgba(239,143,60,.16)' } },
  { id: 'mint', name: '민트 카페', bg: 'mint', light: true, vars: {
    '--bg':'#eef7f2','--bg2':'#e4f1ea','--panel':'rgba(255,255,255,.90)','--panel2':'rgba(244,251,247,.95)',
    '--line':'#d3e7dd','--text':'#234038','--muted':'#6f9387','--accent':'#2bb39a','--accent2':'#199883','--good':'#2f9e63','--glow':'rgba(43,179,154,.16)' } },
  { id: 'bakery', name: '크림 베이커리', bg: 'bakery', light: true, vars: {
    '--bg':'#faf2e2','--bg2':'#f3e7cf','--panel':'rgba(255,251,243,.90)','--panel2':'rgba(250,242,226,.95)',
    '--line':'#e9d8b8','--text':'#4a3722','--muted':'#9c855f','--accent':'#d99a3c','--accent2':'#c47e23','--good':'#2f9e63','--glow':'rgba(217,154,60,.16)' } },
  { id: 'garden', name: '정원', bg: 'garden', light: true, vars: {
    '--bg':'#eef6e6','--bg2':'#e3efd6','--panel':'rgba(255,255,253,.90)','--panel2':'rgba(245,251,238,.95)',
    '--line':'#d7e7c3','--text':'#2c3d1f','--muted':'#6f8a59','--accent':'#5aa544','--accent2':'#46892f','--good':'#2f9e63','--glow':'rgba(90,165,68,.16)' } },
  { id: 'night', name: '야시장', bg: 'night', light: false, vars: {
    '--bg':'#1a1726','--bg2':'#221c30','--panel':'rgba(36,30,48,.80)','--panel2':'rgba(44,36,58,.88)',
    '--line':'#3d3350','--text':'#f3ead9','--muted':'#b3a6c4','--accent':'#ffb455','--accent2':'#ff8f4c','--good':'#7fe0a0','--glow':'rgba(255,180,85,.22)' } },
]
