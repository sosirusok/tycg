...
function petsHTML() {
  ...
  const owned = ownedKeys.map(k => { const { grade, star } = parsePetKey(k), g = PET_GRADE_BY_ID[grade], cnt = s.pets[k], eqc = eq.filter(x => x === k).length; return `<div class="petcard" style="--rc:${g.color}"><div class="pet-cnt">×${cnt}</div><div class="pet-ico">${img(PET_ART[g.art], '')}</div><div class="pet-name">${g.name} ${star}성</div><div class="pet-stats">${petStatsLine(grade, star)}</div><div class="pet-acts"><button class="petbtn ${eqc ? 'on' : ''}" data-act="petequip" data-key="${k}">장착${eqc ? ' ·' + eqc : ''}</button><button class="petbtn ${cnt >= 2 ? '' : 'cant'}" data-act="petfuse" data-key="${k}">합성</button></div></div>` }).join('') || '<div class="empty">보유한 펫이 없어요. 아래 상점에서 구매!</div>'
